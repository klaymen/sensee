'''
sensee - webserver application based on WSGI - 2016
'''
...
config.read( '/etc/sensee/sensee.conf' )
...
#All URLs are prefixed with sensee and it's handled differently on Apache and stand-alone wsgi server mode
...
def senseeApplication( environ, startResponse ):
    '''main function of the sensee'''
    ...
    if SERVERISAPACHE is False:
        if urlPath[0].lower() in STATIC:
            ...
            if os.path.exists(path):                                                                #it exists?
                if not os.path.isfile(path):                                                        #is it a folder?
                    logger.debug('static content is a folder: %s' % str(path))
    ...
    ##### WSGI INPUT #####        
    # Saving wsgi.input as StringIO (handling as a file) since it can be read only once
    ...
    # seek back to the first byte of wsgiInputStorage to have it at hand if it is needed again
    ...
    try:
        ...
        logger.error('  unable to get authentication status from config')
    ...
    if urlPath[0].lower() == 'debug' and accessLevel == RW_ACCESS:
        ...
        if not SERVERISAPACHE:
            if urlPath[0] != 'sensee' or urlPath == []:
                urlPath.insert(0, 'sensee')
...
if __name__ == '__main__':
    try:
        ...
        print 'Running sensee application at http://%s:%s/ ...' % ( str(WEBSERVERHOST), str(WEBSERVERPORT) )
